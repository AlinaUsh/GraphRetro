...
class LGClassifier(nn.Module):
    ...
    def predict(self, prod_smi: str, core_edits: List, rxn_class: int = None, mcd=0):
        ...
        rxn_class???
        mcd: int,
            Number of MC samples (0 -- without MC)
        """
        ...
        with torch.no_grad():
            ...
            if fragments is None:
                ...
                print(f'lg_shared_embed logits_shape: {lg_logits.shape}')

                _, preds = torch.max(lg_logits, dim=-1)
                preds = preds.squeeze(0)
                pred_labels = [self.lg_vocab.get_elem(pred.item()) for pred in preds]

                print(f'lg_shared_embed pred_labels_shape: {len(pred_labels)}')
                print()
